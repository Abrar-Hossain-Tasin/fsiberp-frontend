...
const IncidentTypeCheckboxes = ({ typeOfIncident, setTypeOfIncident }) => {
  ...
  return (
    <form className="flex gap-1 flex-wrap text-sm py-2 font-[500]">
      {checkboxOptions.map((option) => (
        <label key={option.value} className="mr-4">
          <input
            ...
            type="checkbox"
            value={option.value}
            checked={typeOfIncident.includes(option.value)}
          />
          {option.label}
        </label>
      ))}
...
export default IncidentTypeCheckboxes;
