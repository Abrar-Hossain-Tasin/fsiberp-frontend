...
const BmsAdminDashboard = () => {
  ...
  const handleRejected = () => {
    setisOpenRejected((prev) => !prev);
    setIsOpenPending(false);
    setIsOpenAccepted(false);
  };
  const handleAccepted = () => {
    setIsOpenAccepted((prev) => !prev);
    setisOpenRejected(false);
    setIsOpenPending(false);
  };
  ...
  const handleSelectedRowSubmit = async () => {
    try {
      ...
      const response = await fetch(
        `${bmsApiEightyOne}/api/bms/approval/${decryptedUserId}/${decryptedFormId}/${decryptedId}`,
        {
          method: "PUT",
          headers: {
          ...
          <div className="relative overflow-hidden shadow-[0_5px_10px_0px_gray] sm:rounded-lg bg-white p-5">
            ...
            <div className="flex justify-center">
              <button
                ...
                className="mt-4 py-2 px-4 bg-green-700 border-2 text-white rounded hover:font-bold hover:border-green-700 hover:bg-green-100  hover:text-black"
              >
                Download PDF
              </button>
            </div>
