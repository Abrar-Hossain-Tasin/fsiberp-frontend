  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        username, designation, contactno, email, dateofjoining;
      } catch (error) {
        setError(error);
      } finally {
        setLoading(false);
    ...
    <form className="m-auto">
      ...
      <div className="bg-white rounded-xl shadow-[0_5px_10px_0px_gray] my-5 w-[900px]">
        <div className="mx-7 flex flex-col">
          ...
          />
          <div className="mt-3">
            {<UserLoginDetails userid={decryptId(userId)} />}
          </div>
            ...
            <div className="mt-3">
              ...
              <div className="flex mb-4">
                <div className="flex flex-col w-full border border-black pb-3">
                  ...
                  {formData.documentDownloadUrl &&
                  formData.documentDownloadUrl.length > 0 ? (
                    <ol className="flex list-none justify-center p-0 mt-2  ">
                      {" "}
                      {/* Flex to arrange items inline */}
                      {formData.documentDownloadUrl.map((url, index) => (
                        <li key={index} className="mr-4 ">
                          {" "}
                          {/* Margin right for spacing */}
                          <Link
                            to={url}
                            className="font-medium text-[15px] rounded-md shadow-md p-1 bg-green-300 hover:bg-green-400 active:bg-green-500"
                            target="_blank" // Opens in a new tab
                            rel="noopener noreferrer" // Security best practice
                          >
                            {`${index + 1}. Attachment ${index + 1}`}{" "}
                            {/* Display attachment number */}
                          </Link>
                        </li>
                      ))}
                    </ol>
              ...
              <div className="flex mb-4">
                <div className="flex flex-col w-full border border-black pb-3">
                  ...
                  formData.documentDownloadUrl.length > 0 ? (
                    <ol className="flex list-none justify-center p-0 mt-2  ">
                      ...
                      {formData.documentDownloadUrl.map((url, index) => (
                        <li key={index} className="mr-4 ">
                          ...
                          <Link
                            to={url}
                            className="font-medium text-[15px] rounded-md shadow-md p-1 bg-green-400 hover:bg-green-500 active:bg-green-600 transition-all duration-300 ease-in-out"
                            target="_blank" // Opens in a new tab
                            rel="noopener noreferrer" // Security best practice
                          >
                            {`${index + 1}. Attachment ${index + 1}`}{" "}
                    ...
                    <span>No document has been uploaded.</span>
                  )}
                </div>
              </div>
            </div>
