...
const AccessToDatabaseTableView = ({
  accesstodatabase,
  setAccesstodatabase,
  type,
}) => {
  ...
  // Initialize rows based on accesstodatabase
  useEffect(() => {
    const initialRows = [];
    for (let i = 0; i < accesstodatabase.length; i += 5) {
      initialRows.push({
        inputs: accesstodatabase.slice(i, i + 5),
      });
    }
    setRows(initialRows);
  }, [accesstodatabase]);
  ...
  const handleInputChange = (rowIndex, index, event) => {
    ...
    // Update accesstodatabase immediately
    const updatedAccesstodatabase = newRows.flatMap((row) =>
      row.inputs.map((input) => (input.trim() === "" ? "" : input))
    );
  ...
  const handleRemoveRow = (rowIndex) => {
    ...
    // Update accesstodatabase when a row is removed
    const updatedAccesstodatabase = newRows.flatMap((row) =>
      row.inputs.map((input) => (input.trim() === "" ? "" : input))
    );
              ...
              {row.inputs.map((input, index) => (
                <td key={index} className="border border-black p-1">
                  <input
                    ...
                    className="text-[#3b3838] text-center w-full border-[1px] border-black p-1 rounded-md font-semibold placeholder-gray-300 focus:outline-none focus:border-green-700 shadow-md transition duration-200 ease-in-out text-sm placeholder-opacity-100"
                    disabled={type === "view"}
                  />
                </td>
              ))}
...
export default AccessToDatabaseTableView;
