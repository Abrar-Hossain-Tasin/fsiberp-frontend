...
const WitnessViewEdit = ({ setHandleWitness, handleWitness, type, userId }) => {
  ...
  // // Update the rows based on handleWitness
  // useEffect(() => {
  //   const initialRows = handleWitness.map((userId) => {
  ...
  //   });
  //   setRows(initialRows);
  // }, [handleWitness, eligibleUsers]); // Re-run if handleWitness or eligibleUsers change

  useEffect(() => {
    const initialRows = handleWitness.map((item) => {
      let name = "";
      let userId = null;
  ...
  }, [eligibleUsers]); // Re-run if handleWitness or eligibleUsers change
  ...
  const handleInputChange = (e, index) => {
    ...
    // Update handleWitness with the merged array
    setHandleWitness(updatedName);
    // (handleWitness);

    // Debug: Log to check the final value of handleWitness
    // (updatedName);
  };
  ...
  const handleOptionClick = (
    ...
    setHandleWitness(updatedRows.map((row) => row.name)); // Pass the userIds to the parent
  };
  ...
  const updateSelectedUserIds = (updatedRows) => {
    ...
    setHandleWitness(userIds); // Update handleByUserId state in the parent
  };
...
export default WitnessViewEdit;
