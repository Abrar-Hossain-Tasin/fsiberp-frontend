...
const BoardMemoDashboard = () => {
  ...
  const fetchData = async (type) => {
    try {
      const response = await fetch(
        `${Base_api}/api/boardmemo/user/${type}/${userid}`
      );
  ...
  return (
    <div className="flex flex-col w-10/12 items-center">
      <h1 className="text-3xl font-bold mt-2 text-center">Memo Dashboard</h1>
...
const Table = ({ data }) => {
  ...
  const handleEdit = (userId, formId, id) => {
    ...
    navigate(
      `../board-memo/edit/${encryptId(userId)}/${encryptId(formId)}/${encryptId(
        id
      )}`
    );
  };
...
export default BoardMemoDashboard;
